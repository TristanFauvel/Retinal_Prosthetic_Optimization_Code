...
try M = load(perceptual_model_filename, 'M');
    M = M.M;
catch
...
%     toc
%     figure(); imagesc(M(:,:,5))
%     D3_matrix_visualization(M)
%     t = timeit(@() double(pymod.Compute_perceptual_model(kwargs)));
    %py.reload('perceptual_model_latest');
    %     Mest = load('perceptual_model.mat');
...
Pr = reshape(magnitude_factor*P, ny,nx);
[Pr,pmax] = vision_model(Mest,W,S);
...
figure()
imagesc(reshape(S*255, ny, nx), clims)
colormap(gray(256));
