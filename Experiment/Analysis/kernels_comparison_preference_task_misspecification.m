...
for i = 1:numel(kernel_list)
    ...
    switch kernelname
        ...
        case 'ARD'
            ...
            theta = [-0.0995, -4.8518,-10.0000,0.6491,-0.9257,0.5564,0.3135];
            kernelname = 'ARD';
    end
...
for k =1:K
    ...
    for i = 1:numel(kernel_list)
        ...
        options_theta = experiment.options_theta;
            ...
            theta = multistart_minConf(@(hyp)negloglike_bin(hyp, x_train_norm_data(:, training_set), c_train_data(:, training_set), model), hyp_lb, hyp_ub,10, init_guess, options_theta);
        end
        
        [mu_c,  mu_y, ~, Sigma2_y] = model.prediction(theta, x_train_norm_data(:, training_set), c_train_data(training_set), x_train_norm_data, post);
        mu_c = mu_c';
