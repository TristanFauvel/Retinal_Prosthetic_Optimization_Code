...
class AxonMapSpatial(SpatialModel):
    ...
    *  The axon map is not very accurate when the upper bound of
    ...
    def _jansonius2009(self, phi0, eye='RE'):
        ...
        if self.engine == 'cython':
            xprime, yprime = fast_jansonius(rho, phi0, self.beta_sup, self.beta_inf)
        else:
            if is_superior:
                # Axon is in superior retina, compute `b` (real number) from
                # Eq. 5:
                b = np.exp(self.beta_sup + 3.9 * np.tanh(-(phi0 - 121.0) / 14.0))
                ...
                # Eq. 6:
                b = -np.exp(self.beta_inf + 1.5 * np.tanh(-(-phi0 - 90.0) / 25.0))
